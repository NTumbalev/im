...
//error_reporting(E_ALL ^ E_NOTICE);	
	...
	define('CONFIG_SESSION_LOGGEDIN_VARNAME', 'AFM_LOGGEDIN');
	...
	*/
	...
	while (true) {
		$path .= '../';
		if(is_dir($path) && (basename(realpath($path)) == 'src' || basename(realpath($path)) == 'vendor' || basename(realpath($path)) == 'web')) {
			define('CONFIG_SYS_DEFAULT_PATH', $path .'../web/uploads/assets/'); //accept relative path only
			define('CONFIG_SYS_ROOT_PATH', $path .'../web/uploads/assets/');	//accept relative path only
			break;
		} 
	}
	// define('CONFIG_SYS_DEFAULT_PATH', dirname(__FILE__) .'/../../../../../../uploads/assets/'); //accept relative path only
	// define('CONFIG_SYS_ROOT_PATH',    dirname(__FILE__) .'/../../../../../../uploads/assets/');	//accept relative path only
