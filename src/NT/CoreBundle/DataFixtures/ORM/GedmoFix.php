    ...
    public function loadExtensionMetadata(ClassMetadata $meta, ExtensionMetadataInterface $exm)
    {
        ...
        if (isset($xmlDoctrine->field)) {
            ...
            foreach ($xmlDoctrine->field as $mappingDoctrine) {
                $xmlMapping = $mappingDoctrine->children(self::GEDMO_NAMESPACE_URI);

                $field = $this->getAttribute($mappingDoctrine, 'name');
                if (isset($xmlMapping->{'tree-left'})) {
                    $mapping['left'] = $field;
                } elseif (isset($xmlMapping->{'tree-right'})) {
                    $mapping['right'] = $field;
                } elseif (isset($xmlMapping->{'tree-root'})) {
                    $mapping['root'] = $field;
                } elseif (isset($xmlMapping->{'tree-level'})) {
                    $mapping['level'] = $field;
                } elseif (isset($xmlMapping->{'tree-path'})) {
                    $separator = $this->getAttribute($xmlMapping->{'tree-path'}, 'separator');
                    $appendId = $this->getAttribute($xmlMapping->{'tree-path'}, 'append_id');
                    if (!$appendId) {
                        $appendId = true;
                    } else {
                    ...
                    $startsWithSeparator = $this->getAttribute($xmlMapping->{'tree-path'}, 'starts_with_separator');
                    ...
                    $endsWithSeparator = $this->getAttribute($xmlMapping->{'tree-path'}, 'ends_with_separator');
                ...
                } elseif (isset($xmlMapping->{'tree-path-source'})) {
                    $mapping['path_source'] = $field;
                } elseif (isset($xmlMapping->{'tree-lock-time'})) {
                    $mapping['lock'] = $field;
                }
            }
        }
