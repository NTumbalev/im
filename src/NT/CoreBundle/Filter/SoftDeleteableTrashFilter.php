<?php
namespace NT\CoreBundle\Filter;
...
class SoftDeleteableTrashFilter extends SQLFilter
{
    ...
    public function addFilterConstraint(ClassMetadata $targetEntity, $targetTableAlias)
    {
        ...
        if (!($config instanceof \Gedmo\SoftDeleteable\Mapping\SoftDeleteableMetadata)) {
            return '';
        }

        $conn = $this->getEntityManager()->getConnection();
        $platform = $conn->getDatabasePlatform();
        $column = $targetEntity->getQuotedColumnName($config->getField(), $platform);
        $addCondSql = $platform->getIsNotNullExpression($targetTableAlias.'.'.$column);

        if ($config->isTimeAware()) {
